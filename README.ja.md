...
### Vertex AI / Gemini の接続

エージェントの LLM 呼び出しは（ハッカソン要件である）**Vertex AI** 経由で、
AI Studio ではない。具体的な接続箇所:

- **モデル** — `google-genai` SDK の `vertexai=True` 経由で
  `gemini-2.5-flash` を呼び出し。`GEMINI_MODEL` env var で差し替え可能
  （[`agent/agent.py:12`](./agent/agent.py)）
- **クライアント経路** — `GOOGLE_GENAI_USE_VERTEXAI=true` を起動時に固定
  （[`agent/server.py:38`](./agent/server.py)）。Cloud Run リビジョンにも
  Terraform 経由で同じ env を注入（[`infra/cloudrun.tf:74`](./infra/cloudrun.tf)）
- **リージョン** — `VERTEX_LOCATION`（デフォルト `us-central1`）
- **IAM** — エージェントランタイム SA に `roles/aiplatform.user` を付与
  （[`infra/iam.tf:18`](./infra/iam.tf)）、`aiplatform.googleapis.com` も
  Terraform 管理 API リストに含む（[`infra/services.tf:9`](./infra/services.tf)）

軌跡 embedding は **Vertex AI を使わない**。〜1,500 件と小規模なので、
`workers=1` で決定論的に学習する 128 次元 Word2Vec をローカルで回した方が
速く（〜3 秒）、コストもかからず、再現性の議論もしやすい。学習は
`embedding/` 配下、出力は BigQuery に保存。
